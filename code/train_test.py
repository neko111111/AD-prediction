...
def train_epoch(data_list, label, one_hot_label, sample_weight, adj_list, model_dict, optim_dict, train_VCDN=True):
    ...
    for i in range(num_view):
        ...
        ci_loss = 0
        ci = model_dict["C{:}".format(i+1)](model_dict["B{:}".format(i+1)](data_list[i], adj_list[i], 0.5))  # x = Ci(Ei(Di(x))
    ...
    if train_VCDN and num_view >= 2:
        ...
        ci_list = []
        for i in range(num_view):
            ci_list.append(model_dict["C{:}".format(i+1)](model_dict["B{:}".format(i+1)](data_list[i], adj_list[i], 0.5)))
