...
from model_init import init_model_dict
...
def cal_feat_imp(data_folder, model_folder, view_list, num_class, rep, config, test_AE=True):
    print(f'Now proceed with the round {rep} of biomarker mining:')
    num_view = len(view_list)
    adj_parameter = 2 if num_class == 2 else 5
    ...
    model_dict = init_model_dict(num_view, num_class, dim_list, config['hid_AE'], config['out_AE'], config['dim_k'],
                                 config['dim_v'], config['in_VCDN'], config['hid_VCDN'], test_AE, config['dropout_AE'],
                                 config['dropout_SA'], config['dropout_VCDN'])
    for m in model_dict:
        if cuda:
            model_dict[m].cuda()
    model_dict = load_model_dict(model_folder, model_dict)
    te_prob = test_epoch(data_trte_list, trte_idx["te"], adj_te_list, model_dict, test_AE)
    if num_class == 2:
